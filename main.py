...
# --- FIX DE RUTAS ABSOLUTAS (Para que App.py no se pierda) ---
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
ARCHIVO_CONFIG = os.path.join(BASE_DIR, "config.yaml")
ARCHIVO_ESTADO = os.path.join(BASE_DIR, "state.json") # ### NUEVO: Definimos nombre del archivo de memoria ###
...
def ejecutar_batch_con_reintentos(cursor, sql, lista_datos):
    ...
    for i in range(MAX_REINTENTOS):
        try:
            # extras.execute_batch es mucho más rápido que ejecutar uno por uno
            extras.execute_batch(cursor, sql, lista_datos, page_size=BATCH_SIZE)
            return # Éxito
        except Exception as e:
            try: cursor.connection.rollback()
            except: pass
...
def ejecutar_migracion(rol_web=None, opcion_web=None):
    ...
    if rol_web and opcion_web:
        ...
        # print("\n🔐 SISTEMA DE SEGURIDAD ETL (Punto 8)")
        # Para uso manual sin web
        usuario_rol = "dev"
        opcion = "3"
    ...
    for tabla_info in config['tablas']:
        ...
        # NOTA: Ya no borramos aquí adentro, se hizo arriba en el bloque de limpieza
        ...
        try:
            ...
            # ### PUNTO 8 ###
            # No imprimimos la query si pudiera contener secretos
            # print_log(f"   🔍 Query a ejecutar: {sql_final}") # Opcional: Ocultamos query en consola para limpieza
        ...
        try:
            if nombre_tabla == "clientes":
                ...
                # 3. Enviamos lote con funcion nueva
                if datos_batch:
        ...
        # Visualización rápida en consola para verificar (solo primeros 2 para no saturar)
        if filas_enmascaradas and len(filas_enmascaradas) > 0:
            # Solo imprimimos si no hay demasiados datos para no ensuciar la consola
            if len(filas_enmascaradas) < 5:
                print_log(f"   [Ejemplo Original]: {filas[0]}")
                print_log(f"   [Ejemplo Masking]:  {list(filas_enmascaradas[0].values())}")
